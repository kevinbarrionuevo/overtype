...
`;)d++;i.selectionStart=a,i.selectionEnd=d}else pe(i);let p=e(i);if(t.adjustSelection){let l=i.value.slice(i.selectionStart,i.selectionEnd).startsWith(t.prefix),c=t.adjustSelection(l,n,o,a);p.selectionStart=c.start,p.selectionEnd=c.end}else if(t.prefix){let l=i.value.slice(i.selectionStart,i.selectionEnd).startsWith(t.prefix);r?l?(p.selectionStart=Math.max(n-t.prefix.length,a),p.selectionEnd=p.selectionStart):(p.selectionStart=n+t.prefix.length,p.selectionEnd=p.selectionStart):l?(p.selectionStart=Math.max(n-t.prefix.length,a),p.selectionEnd=Math.max(o-t.prefix.length,a)):(p.selectionStart=n+t.prefix.length,p.selectionEnd=o+t.prefix.length)}return p}function V(i,e){let t,n,{prefix:o,suffix:r,blockPrefix:s,blockSuffix:a,replaceNext:p,prefixSpace:d,scanFor:l,surroundWithNewlines:c,trimFirst:h}=e,u=i.selectionStart,f=i.selectionEnd,m=i.value.slice(i.selectionStart,i.selectionEnd),g=se(m)&&s&&s.length>0?`${s}
`:o,v=se(m)&&a&&a.length>0?`
${a}`:r;if(d){let L=i.value[i.selectionStart-1];i.selectionStart!==0&&L!=null&&!L.match(/\s/)&&(g=` ${g}`)}m=Je(i,g,v,e.multiline);let b=i.selectionStart,k=i.selectionEnd,M=p&&p.length>0&&v.indexOf(p)>-1&&m.length>0;if(c){let L=Q(i);t=L.newlinesToAppend,n=L.newlinesToPrepend,g=t+o,v+=n}if(m.startsWith(g)&&m.endsWith(v)){let L=m.slice(g.length,m.length-v.length);if(u===f){let C=u-g.length;C=Math.max(C,b),C=Math.min(C,b+L.length),b=k=C}else k=b+L.length;return{text:L,selectionStart:b,selectionEnd:k}}else if(M)if(l&&l.length>0&&m.match(l)){v=v.replace(p,m);let L=g+v;return b=k=b+g.length,{text:L,selectionStart:b,selectionEnd:k}}else{let L=g+m+v;return b=b+g.length+m.length+v.indexOf(p),k=b+p.length,{text:L,selectionStart:b,selectionEnd:k}}else{let L=g+m+v;b=u+g.length,k=f+g.length;let C=m.match(/^\s*|\s*$/g);if(h&&C){let te=C[0]||"",ne=C[1]||"";L=te+g+m.trim()+v+ne,b+=te.length,k-=ne.length}return{text:L,selectionStart:b,selectionEnd:k}}}function de(i,e){let{prefix:t,suffix:n,surroundWithNewlines:o}=e,r=i.value.slice(i.selectionStart,i.selectionEnd),s=i.selectionStart,a=i.selectionEnd,p=r.split(`
